...
export const getEstudiantesQuery = (tipo?: string, unidad?: string, ids?: number[], carreras?: string): string => {
  let query = `SELECT `;

  if (unidad) query += `du.nombre, `;
  if (carreras) query += `dc.clave, dc.abreviacion, dc.nombre, `;
